      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {volunteers.length === 0 ? (
          <div className="bg-gray-800 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-500 text-sm mt-1">Register from the Volunteer page</p>
          </div>
        ) : (
            ...
            {volunteers.map(vol => (
              <div key={vol.id} className="bg-gray-800 rounded-lg p-5 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {(vol.skills || []).map(skill => (
                      <span key={skill} className="bg-orange-900 text-orange-300 text-xs px-2 py-1 rounded-full capitalize">
                        {skill}
                      </span>
                    ))}
                  </div>
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${vol.available !== false ? 'bg-green-700 text-green-200' : 'bg-red-700 text-red-200'}`}>
                  {vol.available !== false ? 'Available' : 'Busy'}
                </span>
              </div>
            ))}
          </div>
