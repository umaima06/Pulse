  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'tasks'), (snapshot) => {
      const data = snapshot.docs
        ...
        .sort((a, b) => (b.timestamp?.toMillis() || 0) - (a.timestamp?.toMillis() || 0)) // ✅ YOUR FEATURE
      setTasks(data)
      setLoading(false)
    })
    return () => unsub()
      ...
      <div className="mt-3 p-3 rounded-lg bg-green-900 border border-green-700">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-green-500 text-xs">
            {Math.round((task.proof_confidence || 0) * 100)}%
          </span>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#0f172a,_#020617)] text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {/* STATS (from pulled code) */}
        <div className="flex gap-3 mb-8 flex-wrap">
          {[
            { label: 'ASSIGNED', count: tasks.filter(t => t.status === 'assigned').length },
            { label: 'ACCEPTED', count: tasks.filter(t => t.status === 'accepted').length },
            { label: 'AWAITING PROOF', count: tasks.filter(t => t.status === 'awaiting_proof').length },
            { label: 'VERIFIED', count: tasks.filter(t => t.status === 'done' && t.proof_verified === true).length },
            { label: 'FLAGGED', count: tasks.filter(t => t.proof_verified === false || t.proof_verified === null).length },
          ].map(stat => (
