...
function VolunteerPortal() {
  ...
  const updateTask = async (taskId, status) => {
    ...
    } catch { alert('Could not update task. Make sure backend is running.') }
    setActionLoading('')
  }
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#0f172a,_#020617)] text-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-500 text-sm">Enter your WhatsApp number to see assigned tasks</p>
        </div>

        {!submitted ? (
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-8">
            <label className="text-gray-400 text-xs font-bold tracking-widest uppercase mb-3 block">Your WhatsApp Number</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-8">
              <button onClick={() => { setSubmitted(false); setPhone(''); setTasks([]) }}
                className="text-gray-500 hover:text-emerald-400 text-sm transition-colors font-medium">← Search again</button>
            ...
            {loading ? (
              <div className="flex flex-col items-center py-16">
                ...
                <p className="text-emerald-400 text-sm tracking-widest animate-pulse">Looking up tasks...</p>
              </div>
            ) : tasks.length === 0 ? (
              <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-12 text-center">
                <p className="text-6xl mb-5">📭</p>
                <p className="text-gray-300 text-xl font-bold mb-2">No tasks found</p>
                <p className="text-gray-600 text-sm">No tasks assigned to {phone}</p>
              </div>
            ) : (
              <div>
                {/* Mini stats */}
                <div className="grid grid-cols-3 gap-3 mb-8">
                  {[
                    { label: 'ACTIVE', value: activeTasks.length, bg: 'bg-gradient-to-br from-yellow-950/60 to-yellow-900/20', border: 'border-yellow-500/20', text: 'text-yellow-400', dot: 'bg-yellow-400' },
                    { label: 'COMPLETED', value: doneTasks.length, bg: 'bg-gradient-to-br from-emerald-950/60 to-emerald-900/20', border: 'border-emerald-500/20', text: 'text-emerald-400', dot: 'bg-emerald-400' },
                    { label: 'TOTAL', value: tasks.length, bg: 'bg-gradient-to-br from-gray-900/80 to-gray-800/40', border: 'border-white/10', text: 'text-white', dot: 'bg-gray-400' },
                  ].map(s => (
                      ...
                      {activeTasks.map(task => (
                        <div key={task.id} className="bg-gradient-to-br from-gray-900/80 to-gray-800/50 backdrop-blur-sm border border-emerald-500/20 rounded-2xl p-6 hover:shadow-[0_0_25px_rgba(16,185,129,0.1)] transition-all relative overflow-hidden">
                          ...
                          <div className="flex items-start justify-between mb-5 pl-3">
                            <div>
                              <p className="text-white font-black text-lg capitalize">{task.need_type || 'Crisis Response'} Task</p>
                          ...
                          {task.urgency && (
                            <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 ml-3">
                              <div className="flex justify-between items-center mb-2">
                                <p className="text-gray-500 text-xs font-bold tracking-widest uppercase">Urgency Level</p>
                              ...
                              <div className="w-full bg-gray-800 rounded-full h-2 overflow-hidden">
                                <div className="h-2 rounded-full transition-all duration-700"
                                  style={{ width: task.urgency + '%', backgroundColor: task.urgency >= 80 ? '#ef4444' : task.urgency >= 50 ? '#f97316' : '#eab308', boxShadow: '0 0 6px currentColor' }} />
                              </div>
                            </div>
                          )}
                ...
                {doneTasks.length > 0 && (
                  <div>
                    <h3 className="text-gray-500 font-bold mb-4 text-sm tracking-widest uppercase">📜 Task History</h3>
                    <div className="space-y-3">
                      {doneTasks.map(task => (
                        <div key={task.id} className="bg-gray-900/40 border border-white/5 rounded-xl p-4 opacity-50 flex items-center justify-between">
                          <p className="text-gray-400 capitalize font-medium text-sm">{task.need_type || 'Crisis Response'} Task</p>
                          <span className="bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 px-3 py-1 rounded-xl text-xs font-bold">✅ DONE</span>
                        </div>
                      ))}
                    </div>
                  </div>
