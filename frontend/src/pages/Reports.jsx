  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        <h2 className="text-3xl font-bold mb-2">Incoming Reports</h2>
        <p className="text-gray-400 mb-8">Live field reports from WhatsApp</p>
        ...
        {reports.length === 0 ? (
          <div className="bg-gray-800 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-500 text-sm mt-1">Reports appear when WhatsApp messages come in</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex gap-2 mb-2">
                      {report.need_type && <span className="bg-gray-700 text-gray-200 text-xs px-2 py-1 rounded-full capitalize">{report.need_type}</span>}
                      {report.language && <span className="bg-gray-600 text-gray-300 text-xs px-2 py-1 rounded-full">{report.language}</span>}
                    </div>
                    {report.summary && <p className="text-white text-sm mb-2">{report.summary}</p>}
                    {report.raw_message && <p className="text-gray-400 text-xs italic">"{report.raw_message}"</p>}
                    {(report.location_lat && report.location_lng) && (
                      <p className="text-gray-500 text-xs mt-2">📍 {report.location_lat?.toFixed(4)}, {report.location_lng?.toFixed(4)}</p>
                    )}
                  </div>
                  <div className="ml-4 text-right">
                    <p className={`text-2xl font-bold ${getUrgencyColor(report.urgency_score)}`}>{report.urgency_score || '?'}</p>
