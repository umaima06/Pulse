  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-lg mx-auto px-6 py-10">
        <h2 className="text-3xl font-bold mb-2">{t('volunteer_title')}</h2>
        <p className="text-gray-400 mb-8">{t('volunteer_sub')}</p>

        {status === 'success' && (
          <div className="bg-green-800 border border-green-500 rounded-lg p-4 mb-6">
            <p className="text-green-300 font-medium">{t('register_success')}</p>
          </div>
        )}
        {status === 'error' && (
          <div className="bg-red-900 border border-red-500 rounded-lg p-4 mb-6">
            <p className="text-red-300 font-medium">{t('register_error')}</p>
          </div>
        )}

        <div className="space-y-5">
          <div>
            <label className="text-gray-300 text-sm mb-1 block">{t('full_name')}</label>
          ...
          <div>
            <label className="text-gray-300 text-sm mb-1 block">{t('org_name')}</label>
          ...
          <div>
            <label className="text-gray-300 text-sm mb-1 block">{t('email_label')}</label>
            <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
              placeholder="ali@email.com"
              className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-orange-400" />
          </div>
          <div>
            <label className="text-gray-300 text-sm mb-1 block">{t('whatsapp')}</label>
            <input type="tel" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })}
              placeholder="+91 9876543210"
              className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-orange-400" />
          </div>
          <div>
            <label className="text-gray-300 text-sm mb-1 block">{t('city')}</label>
            <input type="text" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })}
              placeholder="Hyderabad, Telangana"
              className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-orange-400" />
          </div>
          <div>
            <label className="text-gray-300 text-sm mb-2 block">{t('skills_label')}</label>
